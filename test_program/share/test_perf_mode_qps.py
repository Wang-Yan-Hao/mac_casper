...
from config import CASPER_CONFIG_QPS, DEFAULT_ITERATIONS, run_program, parse_qps

SHOW_PLOTS = "off"

def run_benchmark(service, iterations):
    config = CASPER_CONFIG_QPS.get(service)
    if config is None:
...
def main():
    ...
    if not service_to_run:
        service_to_run = list(CASPER_CONFIG_QPS.keys())
