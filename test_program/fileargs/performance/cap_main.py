...
def main():
    iterations = 50
    functions = ["fileargs_open", "fileargs_lstat", "fileargs_realpath", "fileargs_fopen"]
    results = {f: [] for f in functions}

    for i in range(iterations):
        print(f"\nRunning iteration {i + 1} of fileargs_perf...")
        output = run_program(["cpuset", "-l", "0", "./fileargs_perf", "test1.txt"])
